...
系統支援「事後綁定」：學生點擊「前往教室寫作業」後，會先輸入 Tutor Promotion code（或留白採用預設導師），綁定成功後由系統進行作業開通。目前支援以下雙模式：
- **自建原生 API 模式 (Native API - 推薦)**：當導師於 Dashboard 設定了 GitHub 組織、樣板專案與 GitHub 權杖後，學生點擊「前往教室寫作業」僅需輸入或確認其 GitHub 帳號，系統即可在 3-8 秒內背景自動建立專案 Repo，並將學生新增為專案協作者 (Collaborator)，避開所有組織邀請卡點。
- **GitHub Classroom 模式 (標準)**：若導師僅提供 GitHub Classroom 邀請連結，系統驗證完成後會自動導向該網址，引導學生 Accept assignment 並接受組織邀請。
...
- 運作機制：直接在 GitHub Classroom 或自建 Organization 專案設定中配置 Webhook，指向 `ingestGithubAutograde` 端點。
- 支援對象：
  - **Classroom 模式**：Repo 命名通常為 `[作業名稱]-[學生帳號]`，Webhook 透過組織設定或專案層級設定觸發。
  - **Native API 模式**：Repo 命名為 `[樣板名稱]-[學生帳號]`，在建立專案時系統會自動設定 Webhook。
