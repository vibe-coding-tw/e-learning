...
- 若需登入失敗重試，主導航列會直接重新觸發 Google 授權
...
### 3. 硬體履約工作流 (Hardware Fulfillment)
平台統一收款，台灣與海外的實體商品則交由當地經銷商 / 履約夥伴出貨。系統負責派單、追蹤、通知與結算，Admin 主要管理履約狀態與異常。
...
- `登入` 入口直接走 Google 帳號授權，備援流程也會在原頁面重新觸發授權，不再依賴獨立登入頁。
...
- 國際付款 / 物流目前只有**部分完成**：
  ...
  - 經銷商履約模型已定義，但 `fulfillment_partners`、派單 UI、跨區結算與 partner 後台仍屬擴展規劃，尚未正式完成。
...
- **[🚚 物流管理 MVP](docs/logistics-mvp.md)**：硬體訂單履約流程與 Admin Logistics 分頁規格。
- **[🤝 經銷商履約與平台收款整合規劃](docs/distributor-fulfillment-model.md)**：平台統一收款、台灣/海外經銷商履約出貨的角色分工、資料欄位與結算規格。
- **[🧭 經銷商履約管理機制規劃](docs/distributor-fulfillment-operations.md)**：平台如何派單、經銷商如何接單/出貨、異常處理與結算機制。
