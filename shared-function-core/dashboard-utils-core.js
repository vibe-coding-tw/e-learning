const { normalizeLegacyId, unitIdsMatch } = require('./id-utils');
...
function getLessonLookupKeys(lesson = {}, options = {}) {
    const { includeDocId = true } = options;
    const keys = new Set();
    const add = (value) => {
        if (!value) return;
    ...
    if (includeDocId) add(lesson.docId);
    add(lesson.sku);
