...
- **Main content tabs (2026-07-14)**: `distributor-portal.html` is split into 5 tabs, one section visible at a time — `.portal-maintab-btn` buttons with `data-maintab="admin|pricing|orders|tutor|settlement"` toggle `.portal-maintab-panel[data-maintab-panel="..."]` wrappers via inline `<script>` at the bottom of the file (pure display/hide, does not touch `public/js/distributor-portal.js` — the unhashed source file; the `<script src>` in the HTML points at whatever hash `node scripts/fingerprint-static-assets.js` last produced, don't hardcode a specific hash anywhere including in this doc):
  ...
  - **Admin tab visibility**: the `admin` tab button starts with a `hidden` class. A `MutationObserver` watches `#portal-distributor-admin-section`'s `.hidden` class (toggled by `public/js/distributor-portal.js` based on `state.portal.role === 'admin'`) and mirrors it onto the tab button; if a non-admin somehow lands on the `admin` tab it auto-redirects to `pricing`.
...
- **履約訂單 tab / Fulfillment status (2026-07-14)**: `#portal-fulfill-status` dropdown has 8 stages (`PENDING`/`ASSIGNED`/`ACCEPTED`/`PACKING`/`SHIPPED`/`DELIVERED`/`EXCEPTION`/`CANCELLED`, per `docs/distributor/distributor-fulfillment-operations.md` §7), written to `orders.fulfillmentStatus` via the `paymentUpdateOrderFulfillmentStatus` callable, which validates against `ALLOWED_FULFILLMENT_STATUSES` in `functions-payment/index.js` (whitelist only, no enforced transition order — see comment there for why). Status badge colors in the order table: SHIPPED/DELIVERED = green, PENDING/ASSIGNED/ACCEPTED/PACKING = amber, EXCEPTION = red, CANCELLED = gray. 「待出貨」stat card counts anything not yet `SHIPPED`/`DELIVERED`/`CANCELLED`, not just `PENDING`.
