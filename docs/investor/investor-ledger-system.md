...
## 1. Goal
把投資人計劃變成可執行的系統模組：
- 每一筆收入 / 支出都形成可追蹤的 event，並可自動推動 current balance sheet snapshot
...
- `upsertBalanceSheetSnapshot` 用於鎖定或手動修正資產負債快照；`recordInvestorFinanceEvent` 會同步推動 current snapshot。
