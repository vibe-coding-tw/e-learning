...
- 自動評分系統已全面轉移至平台原生作業 Repo / 教材 Repo 的 Autograding Webhook 機制。
- Workflow 定義與觸發條件由各教材 repo 或學生 repo 維護，這個 root repo 不包含 `autograde-and-sync.yml`。
- Autograding 會在學生 Push 程式碼時自動觸發測試。
- 評分完成後，repo 會發送 Webhook (HTTP POST) 到平台的 `ingestGithubAutograde` 雲端函式。
- Backend:
...
1. 進入對應的教材 repo 或學生作業 repo。
2. 進入 repository 的 workflow / Actions 設定。
