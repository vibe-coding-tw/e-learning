...
> Historical note: this document captures the older expansion roadmap and still references transitional course-key / track concepts. The current canonical course schema and category-first runtime are now defined in `docs/database.md` and the course docs under `docs/courses/`.
...
- Top nav learning-path menu is generated dynamically from Firestore `metadata_settings/learning_paths.categoryLabels`:
  - grouping key source: canonical path key
  - label source priority:
    1. `categoryLabels.{pathKey}.zh-TW`
    2. `categoryLabels.{pathKey}.en`
    3. shared locale-aware fallback only when Firestore data is unavailable
  - `nav-component.js` 與 `learning-path.html` 必須共用同一份 label resolver，避免頁面標題與下拉選單各自 fallback。
...
- [Distributor, Tutor, and Pricing Engineering Spec](../distributor/distributor-tutor-pricing-engineering-spec.md)
