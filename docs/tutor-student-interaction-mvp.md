...
課程單元頁右下角提供單一浮動入口「師生互動與卡點支援」。

1. 點擊按鈕，開啟互動面板（不跳頁）。
2. 查看目前學習狀態節點：`進行中 -> 遭遇卡點 -> 導師引導中 -> 已解決/評分`。
3. 若遇到問題，填寫卡點類型（`concept/debug/environment`）與描述後送出。
4. 依導師 `nextAction` 修正程式並再次 push。
5. 若已自行解決，可在面板手動標記「回到進行中」。
6. 可由面板直接開啟 Dashboard，查看分數、卡點與指導紀錄。

> 備註：原本獨立圖表圓形浮動鈕已整合進此單一入口，避免重疊與操作混淆。
