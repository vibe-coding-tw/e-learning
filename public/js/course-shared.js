...
function getCourseSharedAuth() {
    return window.vibeAuth || window.__vibeAuth || null;
}
...
function isEnLikeCourseLocale(locale = '') {
    return String(locale || '').toLowerCase().startsWith('en');
}

function getCourseUiCopy(locale = getActiveCourseLocale()) {
    const isEn = isEnLikeCourseLocale(locale);
    return isEn ? {
        githubAssignment: 'Go to GitHub Assignment',
        connectGithubTitle: 'Connect your GitHub account',
        connectGithubDesc: 'The system will automatically create a private assignment repository for this unit and add you as a collaborator. Please enter your GitHub username to link it.',
        githubUsernameLabel: 'Your GitHub username',
        githubUsernamePlaceholder: 'e.g. octocat',
        githubUsernameError: 'Invalid username format. Please enter a valid GitHub username.',
        createRepoBtn: 'Create assignment repository',
        cancelBtn: 'Cancel',
        creatingTitle: 'Creating your assignment',
        creatingDesc: 'Calling the GitHub API to clone the template, configure collaborators, and initialize the PR workflow. This usually takes 3-8 seconds. Please do not close this window.',
        successTitle: 'Assignment repository created!',
        successDesc: 'Your private assignment repository is ready. We have also prepared a Feedback Pull Request for future review and discussion.',
        openGithubBtn: 'Go to GitHub Assignment',
        openPrBtn: 'Open Feedback PR discussion',
        closeBtn: 'Close',
        failureTitle: 'Assignment setup failed',
        failureDesc: 'An error occurred while creating the repository. Please check whether your GitHub account exists, or contact your teacher/admin for help.',
        blockerResolved: '✅ Blocker resolved. Happy coding!',
        blockerConfirm: 'Are you sure the blocker is resolved? This will switch the status back to “In progress”.',
        processingBtn: 'Processing...',
        blockerFailed: 'Action failed:',
        blockerRetryBtn: '✅ Mark blocker resolved',
        summaryPrompt: 'Please fill in the blocker/attempt notes before submitting.',
        summarySubmitBtn: 'Submitting...'
    } : {
        githubAssignment: '前往 GitHub 寫作業',
        connectGithubTitle: '連結 GitHub 帳號',
        connectGithubDesc: '系統將自動為您建立該單元的私有作業 Repository 並加您為協作者，請輸入您的 GitHub 帳號（Username）以進行綁定。',
        githubUsernameLabel: '您的 GitHub 帳號名稱',
        githubUsernamePlaceholder: '例如：octocat',
        githubUsernameError: '※ 帳號名稱格式不正確，請輸入正確的 GitHub 帳號名稱。',
        createRepoBtn: '建立專屬作業庫',
        cancelBtn: '取消',
        creatingTitle: '正在建立專屬作業',
        creatingDesc: '正在調用 GitHub API 為您複製樣板、設定協作者與初始化 PR 機制，此過程大約需要 3-8 秒，請勿關閉視窗...',
        successTitle: '🎉 作業庫建立成功！',
        successDesc: '您的個人私有作業 Repository 已成功開通！我們已為您預先開啟了 Feedback Pull Request，可用於未來的代碼審查與討論。',
        openGithubBtn: '前往 GitHub 寫作業',
        openPrBtn: '打開 Feedback PR 討論區',
        closeBtn: '關閉',
        failureTitle: '作業開通失敗',
        failureDesc: '建立時發生錯誤，請確認您的 GitHub 帳號是否存在，或聯繫老師/管理員協助。',
        blockerResolved: '✅ 卡點已解決，祝您寫作業順利！',
        blockerConfirm: '確定卡點已解決嗎？這會將狀態切回「進行中」。',
        processingBtn: '處理中...',
        blockerFailed: '操作失敗：',
        blockerRetryBtn: '✅ 卡點已自行解決',
        summaryPrompt: '請先填寫卡點/嘗試說明，再提交紀錄。',
        summarySubmitBtn: '提交中...'
    };
}
...
function upgradeLegacyUnitToMsLayout() {
    try {
        ...
        const pageTitle = (document.querySelector('header h1')?.textContent || document.title || _t('page_title_fallback', '課程單元')).trim();
        ...
        pageIndex.innerHTML = `
            <div class="unit-content">
                ...
                <div style="margin-top:32px;">
                    <button class="ms-btn" onclick="goToUnit(1)">${_t('btn_start_unit')}</button>
                </div>
            </div>
        `;
...
function injectDashboardModal() {
    ...
    <div id="dashboard-modal-overlay" class="fixed inset-0 bg-black/70 hidden flex-col overflow-hidden" style="z-index: 1000002 !important;">
        <div class="flex items-center justify-between px-4 md:px-6 py-3 bg-white/95 border-b border-slate-200">
            <div class="text-sm md:text-base font-bold text-slate-700">${_t('dash_header_title', '儀表板 (Dashboard)')}</div>
...
async function initFirebaseFeatures() {
    ...
    const { firebaseConfig, connectFirebaseEmulators } = await import("./firebase-local.js?v=3");
        ...
        window.vibeAuth = auth;
        window.__vibeAuth = auth;

        window.__vibeAuthReadyPromise = new Promise((resolve) => {
            let settled = false;
            onAuthStateChanged(auth, (user) => {
                window.__vibeCurrentAuthUser = user || null;
                if (settled) return;
                settled = true;
                resolve(user || null);
            });
        ...
        window.firebaseSubmitAssignment = async (data) => {
             const user = auth.currentUser;
             if (!user) {
                 alert(_t('alert_login_first', '請先登入 (Please Login First)'));
             ...
             try {
                 ...
                 alert(_t('alert_submit_failed_prefix', '提交失敗：') + error.message);
                 throw error;
             }
        };
        ...
        window.submitStudentBlockerAction = async function() {
            ...
            if (!note) {
                alert(copy.summaryPrompt);
                return;
            }
            btn.disabled = true;
            btn.innerText = copy.summarySubmitBtn;
        ...
        window.resolveStudentBlockerAction = async function() {
            const copy = getCourseUiCopy();
            if (!confirm(copy.blockerConfirm)) return;
            const btn = document.getElementById('btn-resolve-blocker');
            btn.disabled = true;
            btn.innerText = copy.processingBtn;
            ...
            try {
                ...
                alert(copy.blockerResolved);
                location.reload();
            } catch (e) {
                console.error(e);
                alert(`${copy.blockerFailed}${e.message}`);
                btn.disabled = false;
                btn.innerText = copy.blockerRetryBtn;
            }
        };
...
function enhanceAssignmentEntryButtons() {
    ...
    groupMap.forEach((items, groupEl) => {
        ...
        btnNative.innerHTML = `
            ...
            <span>${getCourseUiCopy().githubAssignment}</span>
...
window.submitBindTutorAction = async function () {
    ...
    btn.disabled = true;
    btn.innerHTML = _t('btn_verifying_code', '正在驗證代碼...');
    ...
    try {
        ...
        if (result.data && result.data.success) {
            ...
            alert(_t('alert_bind_failed', '❌ 綁定失敗：') + (result.data.message || _t('unknown_error', '未知錯誤')));
        }
    } catch (e) {
        console.error("Binding error:", e);
        alert(_t('alert_error_prefix', '❌ 錯誤：') + e.message);
    } finally {
        btn.disabled = false;
        btn.innerHTML = `
            <span>${_t('tutor_confirm_btn', '確認並前往作業')}</span>
...
window.openSubmissionModal = async function (assignmentId, title, options = {}) {
    ...
    if (!currentUser) {
        alert(_t("alert_login_required", "請先登入後再前往 GitHub 作業。請按右上角登入後再試一次。"));
        return;
    }

    const requestedCourseId = await findCourseIdByUnit(fileName);
    let assignmentAccess = null;

    try {
        if (typeof window.firebaseResolveAssignmentAccess === 'function') {
            assignmentAccess = await window.firebaseResolveAssignmentAccess({ courseId: requestedCourseId, unitId: fileName, assignmentId });
            const resolvedCourseId = String(assignmentAccess?.courseId || requestedCourseId || '').trim();
            ...
            if (!resolvedCourseId) {
                ...
                alert(_t('alert_no_course_data', '已登入，但系統暫時找不到此單元對應的課程資料。請重新整理後再試；若仍無法進入，表示後端課程 metadata 缺少 courseUnits 對應。'));
                return;
            }
            ...
            if (!isAuthorized) {
                alert(_t('alert_no_permission', '尚未取得此單元之付款或導師指派授權。'));
                return;
            }
            ...
            // 觸發原生 API 倉庫建立流程
            await triggerNativeAssignmentCreation(resolvedCourseId, fileName, assignmentId, title, assignmentAccess?.githubUsername);
            return;
        }
    } catch (e) {
        ...
        alert(_t('alert_no_entry', '暫時無法確認您的作業入口，請稍後再試。'));
        return;
    }
};
...
function isAdminTutorModeActive() {
    try {
        const sharedAuth = getCourseSharedAuth();
        const uid = sharedAuth?.currentUser?.uid || window.__vibeCurrentAuthUser?.uid;
        if (!uid) return false;
...
function buildSubmitFailureMessage(rawMessage = '', submitUrl = '') {
    ...
    if (isAssignmentSubmission && maybeOrgInviteIssue) {
        return _t('submit_failed_org_invite', '繳交失敗：{msg}\n\n請先完成以下步驟後再提交：\n1. 檢查您的電子信箱或點擊 GitHub 右上角鈴鐺通知\n2. 接受待處理的作業 Repository 邀請 (Collaborator Invitation)\n3. 回到本頁重新提交').replace('{msg}', message || _t('unknown_error', '尚未完成授權'));
    }
    return _t('submit_failed_prefix', '繳交失敗：') + (message || 'Unknown error');
}
...
window.submitAssignmentAction = async function () {
    ...
    if (!url) {
        alert(_t('alert_enter_link', '請輸入作業連結！'));
        return;
    }
    ...
    btn.disabled = true;
    btn.innerHTML = `<span class="animate-spin inline-block w-4 h-4 border-2 border-white border-t-transparent rounded-full"></span> ${_t('btn_submitting', '提交中...')}`;
    ...
    try {
        ...
        if (result.success) {
            alert(_t('alert_submit_success', '作業繳交成功！老師將會收到通知。'));
    ...
    try {
        ...
        buttons.forEach(btn => {
            ...
            if (onclickAttr.includes('goToUnit(1)') && !btn.classList.contains('nav-btn-prev')) {
                    btn.innerHTML = _t('btn_start_unit');
                }
        });
    } catch (e) {
    ...
    <div id="github-username-modal" class="fixed inset-0 bg-black/60 hidden flex items-center justify-center z-[70] backdrop-blur-sm">
        <div class="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl transform transition-all scale-100 border border-blue-50">
            <div class="text-center mb-6">
                ...
                <h3 class="text-2xl font-black text-gray-800">${copy.connectGithubTitle}</h3>
                <p class="text-sm text-gray-500 mt-2 leading-relaxed">
                    ${copy.connectGithubDesc}
                </p>
            </div>
            
            <div class="mb-6">
                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">${copy.githubUsernameLabel}</label>
                <input type="text" id="github-username-input" placeholder="${copy.githubUsernamePlaceholder}"
                ...
                <p class="text-[10px] text-red-500 mt-2 text-center font-bold hidden" id="github-username-error">${copy.githubUsernameError}</p>
            </div>
            ...
            <div class="flex flex-col gap-3">
                ...
                <button onclick="closeGithubUsernameModal()"
                    class="w-full py-2 text-gray-400 hover:text-gray-600 transition font-medium text-sm">${copy.cancelBtn}</button>
            </div>
        </div>
    </div>
    `;
    ...
    <div id="native-creation-status-modal" class="fixed inset-0 bg-black/60 hidden flex items-center justify-center z-[80] backdrop-blur-sm">
        <div class="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl transform transition-all scale-100 border border-blue-50 text-center">
            ...
            <div id="native-creation-loading-state" class="py-6">
                ...
                <h3 class="text-xl font-bold text-gray-800">${copy.creatingTitle}</h3>
            ...
            <div id="native-creation-success-state" class="py-6 hidden">
                ...
                <h3 class="text-2xl font-black text-gray-805">${copy.successTitle}</h3>
                ...
                <div class="mt-8 flex flex-col gap-3">
                    ...
                    <a id="native-pr-link-btn" href="#" target="_blank"
                        ...
                        <span>${copy.openPrBtn}</span>
                    </a>
                    <button onclick="closeNativeCreationStatusModal()"
                        class="w-full py-2 text-slate-400 hover:text-slate-600 transition font-medium text-sm mt-2">${copy.closeBtn}</button>
                </div>
            </div>
            ...
            <div id="native-creation-failure-state" class="py-6 hidden">
                ...
                <h3 class="text-xl font-bold text-red-600">${copy.failureTitle}</h3>
...
window.executeNativeCreation = async function(githubUsername) {
    ...
    if (!String(courseId || '').trim()) {
        ...
        document.getElementById('native-creation-error-msg').innerText = "缺少課程 ID，無法建立作業 Repository。請重新整理後再試一次。";
        return;
    }
    ...
    try {
        ...
        if (result.data && result.data.repositoryUrl) {
            ...
            throw new Error(_t('error_no_repo_link', '無法取得作業 Repository 連結。'));
        }
    } catch (err) {
    ...
    try {
        ...
        if (isEn) {
            mapTerms = [
                ...
                ['Starter\\s+Unit', 'Starter Course'],
                ['Basic\\s+Unit', 'Basic Course'],
                ['Advanced\\s+Unit', 'Advanced Course'],
