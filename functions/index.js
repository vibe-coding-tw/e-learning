...
let _GitHubAPIHelper = null;
function getGitHubAPIHelper() {
    if (!_GitHubAPIHelper) {
        _GitHubAPIHelper = require('./github-api-helper');
    }
    return _GitHubAPIHelper;
}
const {
...
// Legacy master and unit file mapping to canonical unit files
...
function normalizeLegacyFile(name) {
    if (!name) return name;
    const lower = name.trim().toLowerCase();
    const hasHtml = lower.endsWith('.html');
    const lookupKey = hasHtml ? lower : lower + '.html';
    if (LEGACY_TO_CANONICAL[lookupKey]) {
        const canonical = LEGACY_TO_CANONICAL[lookupKey];
        return hasHtml ? canonical : canonical.replace(/\.html$/i, '');
    }
    return name;
}
...
exports.serveCourse = onRequest({ secrets: [CONTENT_REPO_TOKEN] }, async (req, res) => {
    ...
    // 0. Redirection check for legacy master and unit pages
    {
        const fileName = urlPath.split('/').filter(Boolean).pop();
        if (fileName) {
            const lowerFileName = fileName.toLowerCase();
            const hasHtml = lowerFileName.endsWith('.html');
            const lookupKey = hasHtml ? lowerFileName : lowerFileName + '.html';
            if (LEGACY_TO_CANONICAL[lookupKey]) {
                const canonicalName = LEGACY_TO_CANONICAL[lookupKey];
                const finalName = hasHtml ? canonicalName : canonicalName.replace(/\.html$/i, '');
                const newPath = req.path.replace(new RegExp(fileName + '$', 'i'), finalName);
                const querystring = req.url.includes('?') ? req.url.substring(req.url.indexOf('?')) : '';
                const redirectUrl = newPath + querystring;
                console.log(`[serveCourse] Legacy 302 redirect: ${urlPath} -> ${redirectUrl}`);
    ...
    try {
        ...
        if (isPublicPage) {
            ...
            // Normalize legacy pageId and scopePart to canonical to support backward compatibility
            pageId = normalizeLegacyFile(pageId);
            scopePart = normalizeLegacyFile(scopePart);
...
exports.createStudentRepository = onCall({ secrets: [GITHUB_API_TOKEN] }, async (request) => {
    ...
    try {
        ...
        const GitHubAPIHelper = getGitHubAPIHelper();
