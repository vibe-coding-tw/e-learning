...
function serializeLedgerReportCsv(report = {}) {
    const rows = Array.isArray(report.rows) ? report.rows : [];
    const columnSet = new Set(["reportType", "period"]);
    ...
    const metaRows = [
        { key: "reportId", value: report.reportId || "" },
        { key: "reportType", value: report.reportType || "" },
        { key: "period", value: report.period || "" },
        { key: "currency", value: report.currency || "" },
        { key: "snapshotCount", value: report.snapshotCount ?? "" }
    ];
    const metaHeader = ["key", "value"].join(",");
    const metaBody = metaRows.map((row) => [escapeCsvCell(row.key), escapeCsvCell(row.value)].join(","));
    return [
        "# report metadata",
        metaHeader,
        ...metaBody,
...
async function exportLedgerReport({
    ...
    const normalizedFormat = normalizeLower(format || "csv");
    if (normalizedFormat === "json") {
        return {
            report,
            format: "json",
            contentType: "application/json",
            fileName: `ledger_${normalizeLower(reportType)}_${normalizeText(period)}.json`,
    ...
    return {
        ...
        fileName: `ledger_${normalizeLower(reportType)}_${normalizeText(period)}.csv`,
        content: serializeLedgerReportCsv(report)
    };
}
