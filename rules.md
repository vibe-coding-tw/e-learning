...
- **核心規則**: **嚴禁**向使用者展示任何視覺化的驗證、除錯或排查過程。包括但不限於：
    - 瀏覽器自動化錄影 (Recording)
    - 瀏覽器頁面截圖 (Screenshot)
    - 展示瀏覽器子代理人 (Subagent) 的執行進度
- **執行方式**: AI 代理人應自主在背景（Background）使用工具完成功能驗證與故障排查。完成後，僅需在報告中以簡潔的文字（Text Only）總結驗證結果與排查發現。
- **例外**: 除非使用者在該次請求中明確下令「請截圖讓我看」或「我想看錄影確認 UI」，否則一律禁止主動提供。

---
*最後更新日期: 2026-04-06 (V14.2 - Strict No-Visual Policy)*
