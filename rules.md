...
本區塊定義 AI 代理人（Coding Agent）在開發過程中的行為準則。

### 6-A. 驗證流程精簡 (Verification Streamlining)
- **規則**: **嚴禁**在任務完成後向使用者展示瀏覽器自動化驗證的錄影 (Recording) 或截圖 (Screenshot)。
- **目標**: 節省使用者審閱時間。AI 代理人應自主在背景完成功能驗證，並僅在報告中總結驗證結果即可。
- **例外**: 僅當使用者明確要求「查看視覺效果」或「確認 UI 佈局」時，方可提供視覺化驗證資料。

---
*最後更新日期: 2026-04-06 (V14.1 - AI Standard Update)*
