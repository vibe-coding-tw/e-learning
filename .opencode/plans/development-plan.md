...
| P2.2 | 全面掃描殘留 hardcoded `alert()` / `confirm()`——2026-07-15 掃描結果：`public/js/investor-portal.js` 約 20+ 處硬編碼中文 `alert()`（如 1147/1246/1251/1307 行）、`public/js/distributor-portal.js` 3 處硬編碼 `confirm()`（1128/1146/1203 行，同檔 1305 行已正確用 `window.t()`），`public/js/course-shared.js:1788` 1 處硬編碼英文字串。之前 P1.6/P1.7 只改了 `courses-management.html`/`investor-portal.html` 內的 inline `<script>`，這幾個獨立 `.js` 檔案沒有一併處理。還沒動手改——量偏大（20+ 處），需要獨立一輪處理，不要跟其他 hotfix 一起改以免範圍失控 | 中（比原估計大，原以為是「小」） |
| P2.3 | 檢查 4 個 codebase 的 dangling require/exports——2026-07-15 發現 `functions/index.js` 的 `registerIndexExports(...)` 呼叫被包在 `if (process.env.FUNCTIONS_EMULATOR)` 裡，導致正式 `firebase deploy` 環境下這個 codebase 幾乎所有 export（autograde/payment/admin proxy、`onUserCreated`、`mapReply`、`autoGradingCron`）都不會部署，已修復（移除該 guard）。這比「dangling」更嚴重，是會讓整個 codebase 在生產環境幾乎空的部署阻斷問題，值得之後再過一輪確認沒有類似的環境變數誤用 | 小 → 已抓到 1 個嚴重案例並修復 |
